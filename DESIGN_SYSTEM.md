...
### Badge
```tsx
<Badge label="Групповое" type="filled" icon={<Icon name="people" color="blue" size="xs" />} />
<Badge label="Групповое" type="outline" icon={<Icon name="people" color="blue" size="xs" />} />
```
**Props:** `label`, `type` (filled|outline), `icon`
...
<Player title="Графический дизайнер" state="playing" orientation="horizontal"
  currentTime="01:23" totalTime="03:45" progress={45} />
```
